...
validate_marker <- function(img_matrix, position, min_radius = 3, max_radius = 8, 
  ...
  hip_zone <- position[1] >= 10 && position[1] <= 40 && 
    position[2] >= 140 && position[2] <= 200
  
  knee_zone <- position[1] >= 77 && position[1] <= 95 && 
    position[2] >= 110 && position[2] <= 209
  
  lower_leg_zone <- position[1] >= 105 && position[1] <= 150 && 
    position[2] >= 80 && position[2] <= 239
...
output_dir <- "output_directory_S3_2_5_limp"
dir.create(output_dir, showWarnings = FALSE)

# Set up parameters
video_path <- "S3_2_5_limp.MOV"
