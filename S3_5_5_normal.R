...
find_brightness_threshold <- function(img_matrix, n_top = 10, verbose = FALSE) {
  ...
  hip_threshold <- max(sorted_brightness[1] * 0.85, 0.8)  # Keep this more lenient for hip
  lower_threshold <- max(sorted_brightness[1] * 0.91, 0.8)  # Keep original stricter threshold for lower regions
  
  if(verbose) {
    print("Top brightness values:")
    print(sorted_brightness)
    print(paste("Hip region threshold:", hip_threshold))
    print(paste("Lower region threshold:", lower_threshold))
  }
  
  # Return both thresholds as a list
  return(list(
    hip = hip_threshold,
...
validate_marker <- function(img_matrix, position, min_radius = 3, max_radius = 8, 
  ...
  hip_zone <- position[1] >= 20 && position[1] <= 29 && 
    position[2] >= 138 && position[2] <= 200
  
  knee_zone <- position[1] >= 77 && position[1] <= 94 && 
    position[2] >= 110 && position[2] <= 205
  
  lower_leg_zone <- position[1] >= 105 && position[1] <= 148 && 
    position[2] >= 80 && position[2] <= 238
  ...
  min_brightness <- if(lower_leg_zone) {
    0.40  # More lenient for ankle/foot
  } else if(knee_zone) {
    0.45
  } else {
    0.3  # Stricter for hip
  }
  ...
  # Tweak size for specific region (ensure 
  size_multiplier <- if(position[1] > 140) { # Specifically for foot and greater than y = 140
    0.85  # More lenient for foot
  ...
  circularity_threshold <- if(lower_leg_zone) {
    0.46  # More lenient for ankle/foot
  } else if(knee_zone) {
...
find_markers <- function(img_matrix, brightness_threshold, marker_size = 10, 
  ...
  while(max(img_matrix) > min(brightness_threshold$hip, brightness_threshold$lower) && iteration < max_iterations) {
    ...
    # Use different threshold based on y-position (row)
    current_threshold <- if(brightest_spot[1] <= 50) {  # Hip region
      brightness_threshold$hip
    } else {
      brightness_threshold$lower
    }
    
    # Only proceed if spot is bright enough for its region
    if(max(img_matrix) > current_threshold) {
      ...
      if(brightest_spot[1] > edge_margin && 
        ...
        if(validate_marker(img_matrix, brightest_spot, 
          ...
          if(verbose) print("MARKER REJECTED")
        }
      } else {
...
process_frame <- function(frame_path, target_size = 100, marker_size = 10, verbose = FALSE) {
  ...
  # Choose base sigma based on frame number
  base_sigma <- if(!is.null(special_frames[[as.character(frame_num)]])) {
    special_frames[[as.character(frame_num)]]
  } else {
    0.43  # Default sigma value
  }
  ...
  upper_img <- gray_img %>%
    image_crop(geometry = paste0(dims$width, "x40+0+0")) %>%
    image_blur(radius = 1, sigma = base_sigma * 0.7) # Lower blur for hip region
  
  lower_img <- gray_img %>%
    image_crop(geometry = paste0(dims$width, "x", dims$height-40, "+0+40")) %>%
    image_blur(radius = 1, sigma = base_sigma)
  
  # Combine images
  blurred_img <- image_append(c(upper_img, lower_img), stack = TRUE)
  
  # Process the combined image
  img_matrix <- blurred_img %>%
    image_data(channels = "gray") %>%
    as.numeric() %>%
  ...
  if(verbose) {
    ...
    print(paste("Using base sigma value:", base_sigma))
    print(paste("Using hip region sigma value:", base_sigma * 0.7))
  }
...
output_dir <- "output_directory_S3_5_5_normal"
dir.create(output_dir, showWarnings = FALSE)

# Set up parameters
video_path <- "S3_5_5_normal.MOV"
