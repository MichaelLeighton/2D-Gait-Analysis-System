...
check_marker_size <- function(img_matrix, position, min_radius = 2, max_radius = 5, threshold = 0.5, verbose = FALSE) {
  ...
  if(verbose){
    print(paste("Pixel count:", pixel_count))
    print(paste("Total pixels", total_pixels))
    print(paste("Area ratio:", area_ratio))
    print(paste("Min area ratio:", min_area_ratio))
    print(paste("Max area ratio:", max_area_ratio))
  }
...
validate_marker <- function(img_matrix, position, min_radius = 3, max_radius = 8, 
  ...
  min_brightness <- if(lower_leg_zone) {
    0.20  # More lenient for ankle/foot
  } else if(knee_zone) {
    0.50
  } else {
  ...
  # Tweak size check - can be specific to each zone
  ...
  # Get the adjusted threshold based on brightness
  marker_threshold <- if(center_brightness < 0.25) {
    0.15  # More lenient threshold for dim markers
  } else {
  ...
  size_valid <- check_marker_size(img_matrix, position, 
                                  ...
                                  max_radius * 1.2, 
                                  threshold = marker_threshold, # Pass the adjusted threshold
                                  verbose = verbose)
...
output_dir <- "output_directory_S2_4_PD_edit"
dir.create(output_dir, showWarnings = FALSE)

# Set up parameters
video_path <- "S2_4_PD_edit.MOV"
