...
process_frame <- function(frame_path, target_size = 100, marker_size = 10, verbose = FALSE) {
  ...
  img_matrix <- img %>%
    image_convert("gray") %>%
    image_blur(radius = 1, sigma = 0.40) %>%
    image_data(channels = "gray") %>%
    as.numeric() %>%
...
output_dir <- "output_directory_S3_2_5_normal"
dir.create(output_dir, showWarnings = FALSE)

# Set up parameters
video_path <- "S3_2_5_normal.MOV"
