...
erase_marker <- function(img_matrix, marker_position, radius = 5) {
  # Reduce the erasure radius specifically for the area in question, e.g. the lower leg zone
  if(marker_position[1] >= 110 && marker_position[1] <= 158) {
    radius <- radius * 0.7  # Make erasure radius smaller for ankle region
  }
  ...
  # Use a gradual fade instead of complete erasure
  for(x in x_range) {
    for(y in y_range) {
      dist <- sqrt((x - marker_position[2])^2 + (y - marker_position[1])^2)
      if(dist <= radius) {
        fade_factor <- dist/radius  # 0 at center, 1 at edge
        img_matrix[y, x] <- img_matrix[y, x] * fade_factor
      }
    }
  }
  return(img_matrix)
...
find_brightness_threshold <- function(img_matrix, n_top = 10, verbose = FALSE) {
  ...
  threshold <- max(sorted_brightness[1] * 0.977, 0.8)
...
check_circularity <- function(img_matrix, position, radius = 5, threshold = 0.3, verbose = FALSE, is_lower_leg = FALSE) {
  ...
  min_points <- if(is_lower_leg) 15 else 12
...
check_contrast <- function(img_matrix, position, radius = 3) {
  ...
  contrast_threshold <- if(position[1] > 125) {  # Foot region
    1.1  # More lenient for foot
...
validate_marker <- function(img_matrix, position, min_radius = 3, max_radius = 8, 
  ...
  hip_zone <- position[1] >= 30 && position[1] <= 45 && 
    position[2] >= 163 && position[2] <= 214
  
  knee_zone <- position[1] >= 82 && position[1] <= 105 && 
    position[2] >= 130 && position[2] <= 230
  
  lower_leg_zone <- position[1] >= 125 && position[1] <= 157 && 
    position[2] >= 80 && position[2] <= 264
  ...
  # Tweak center brightness
  min_brightness <- if(lower_leg_zone) {
    0.36  # More lenient for ankle/foot
  } else if(knee_zone) {
    0.50
  } else {
    0.55  # Stricter for hip
  }
  ...
  size_multiplier <- if(lower_leg_zone) {
    0.7  # More lenient for ankle/foot
  } else {
  ...
  circularity_threshold <- if(lower_leg_zone) {
    0.48  # More lenient for ankle/foot
  } else if(knee_zone) {
...
process_frame <- function(frame_path, target_size = 100, marker_size = 10, verbose = FALSE) {
  ...
  img_matrix <- img %>%
    image_convert("gray") %>%
    image_blur(radius = 1, sigma = 0.30) %>%
    image_data(channels = "gray") %>%
    as.numeric() %>%
...
track_markers <- function(current_markers, previous_markers, prev_velocities = NULL) {
  ...
  if(length(previous_markers) == 0) {
    ...
    initial_markers <- list(
      list(name = "hip", num = 4, y = min(markers_df$y)),
      list(name = "knee", num = 3, y = sort(markers_df$y)[2]),
      list(name = "ankle", num = 2, y = sort(markers_df$y)[3]),
      list(name = "foot", num = 1, y = max(markers_df$y))
    )
  ...
  # First pass: assign hip and knee markers (more reliable)
  assignments <- rep(NA, nrow(current_df))
  used_prev_markers <- rep(FALSE, nrow(previous_markers))
  
  # Find the two highest markers (hip and knee)
  upper_markers <- order(current_df$y)[1:2]
  for(i in upper_markers) {
  ...
  # Second pass: handle ankle and foot markers
  lower_markers <- setdiff(1:nrow(current_df), upper_markers)
  
  if(length(lower_markers) == 2) {
    m1 <- lower_markers[1]
    m2 <- lower_markers[2]
    
    # Calculate y-distance between markers
    y_diff <- abs(current_df$y[m1] - current_df$y[m2])
    
    if(y_diff > 5) {  # If there's clear vertical separation
      # Lower marker is foot (1), higher is ankle (2)
      if(current_df$y[m1] > current_df$y[m2]) {
        assignments[m1] <- 1  # foot
        assignments[m2] <- 2  # ankle
      } else {
        assignments[m1] <- 2  # ankle
        assignments[m2] <- 1  # foot
      }
    } else {
      # If markers are close in Y, use previous frame's assignments
      # and distance-based matching
      for(i in lower_markers) {
  ...
  # Handle any remaining unassigned markers
  if(any(is.na(assignments))) {
    ...
    sorted_indices <- unassigned[order(unassigned_y)]
...
output_dir <- "output_directory_S5_2_5_PD"
dir.create(output_dir, showWarnings = FALSE)

# Set up parameters
video_path <- "S5_2_5_PD.MOV"
...
#-------------------------------------------------------------------------------
#### MARKER TRAJECTORY VALIDATION ANALYSIS
#-------------------------------------------------------------------------------

# Function to analyze marker trajectories
analyze_marker_trajectories <- function(marker_data) {
  # Calculate velocities and accelerations
  trajectory_stats <- marker_data %>%
    group_by(marker_num) %>%
    arrange(frame) %>%
    mutate(
      # Velocities (pixels/frame)
      x_velocity = c(NA, diff(x)),
      y_velocity = c(NA, diff(y)),
      velocity = sqrt(x_velocity^2 + y_velocity^2),
      
      # Accelerations (pixels/frame^2)
      x_acceleration = c(NA, diff(x_velocity)),
      y_acceleration = c(NA, diff(y_velocity)),
      acceleration = sqrt(x_acceleration^2 + y_acceleration^2),
      
      # Moving averages for smoothness analysis
      velocity_ma = rollmean(velocity, k = 5, fill = NA),
      acceleration_ma = rollmean(acceleration, k = 5, fill = NA)
    )
  
  # Analyze inter-marker distances
  distances <- marker_data %>%
    group_by(frame) %>%
    summarize(
      hip_knee_dist = dist_between_markers(marker_data, 1, 2),
      knee_ankle_dist = dist_between_markers(marker_data, 2, 3),
      ankle_foot_dist = dist_between_markers(marker_data, 3, 4),
      .groups = 'drop'
    )
  
  # Calculate statistical measures
  trajectory_metrics <- list(
    # Velocity statistics per marker
    velocity_stats = trajectory_stats %>%
      group_by(marker_num) %>%
      summarize(
        mean_velocity = mean(velocity, na.rm = TRUE),
        sd_velocity = sd(velocity, na.rm = TRUE),
        max_velocity = max(velocity, na.rm = TRUE),
        velocity_outliers = sum(abs(scale(velocity)) > 2, na.rm = TRUE)
      ),
    ...
    # Trajectory smoothness (using normalized jerk)
    smoothness = trajectory_stats %>%
      group_by(marker_num) %>%
      summarize(
        normalized_jerk = mean(abs(diff(acceleration)), na.rm = TRUE),
        trajectory_smoothness = 1 / (1 + normalized_jerk)
      )
  )
  
  # Add validation flags
  validation_results <- list(
    # Flag suspicious velocities (>2 SD from mean)
    suspicious_velocities = trajectory_stats %>%
      group_by(marker_num) %>%
      mutate(
        is_outlier = abs(scale(velocity)) > 2
      ) %>%
      filter(is_outlier) %>%
      select(frame, marker_num, velocity),
    
    # Flag inconsistent inter-marker distances (>2 SD from mean)
    inconsistent_distances = distances %>%
      mutate(across(everything(), 
                    ~abs(scale(.x)) > 2,
                    .names = "flag_{.col}"
      )) %>%
      filter(if_any(starts_with("flag_"), ~.x))
  )
  
  return(list(
    metrics = trajectory_metrics,
    validation = validation_results,
    raw_trajectories = trajectory_stats
  ))
}

# Helper function to calculate distance between markers
dist_between_markers <- function(data, marker1, marker2) {
  data %>%
    filter(marker_num %in% c(marker1, marker2)) %>%
    group_by(frame) %>%
    summarize(
      distance = if(n() == 2) {
...
# Function to plot trajectory analysis results
plot_trajectory_analysis <- function(analysis_results) {
  plots <- list()
  
  # Velocity plot
  plots$velocities <- ggplot(analysis_results$raw_trajectories, 
                             aes(x = frame, y = velocity, color = factor(marker_num))) +
    geom_line() +
    geom_point(data = analysis_results$validation$suspicious_velocities,
               size = 3, shape = 21, fill = "red") +
    facet_wrap(~marker_num) +
    labs(title = "Marker Velocities Over Time",
         x = "Frame", y = "Velocity (pixels/frame)",
         color = "Marker") +
    theme_minimal()
  
  # Inter-marker distances
  plots$distances <- analysis_results$metrics$distance_stats %>%
    pivot_longer(everything()) %>%
    ggplot(aes(x = name, y = value)) +
    geom_bar(stat = "identity") +
    coord_flip() +
    labs(title = "Inter-marker Distance Statistics",
         x = "Measurement", y = "Value") +
    theme_minimal()
  
  # Trajectory smoothness
  plots$smoothness <- ggplot(analysis_results$metrics$smoothness, 
                             aes(x = factor(marker_num), y = trajectory_smoothness)) +
    geom_bar(stat = "identity") +
    labs(title = "Trajectory Smoothness by Marker",
         x = "Marker", y = "Smoothness Score") +
    theme_minimal()
...
# Usage example
marker_analysis <- analyze_marker_trajectories(analysis_results$marker_data)
analysis_plots <- plot_trajectory_analysis(marker_analysis)
...
print(marker_analysis$metrics$velocity_stats)
print("\nDistance Consistency:")
print(marker_analysis$metrics$distance_stats)
print("\nTrajectory Smoothness:")
print(marker_analysis$metrics$smoothness)
print("\nNumber of Suspicious Points:")
print(nrow(marker_analysis$validation$suspicious_velocities))
