...
erase_marker <- function(img_matrix, marker_position, radius = 5) {
  # Reduce the erasure radius specifically for the lower leg zone
...
validate_marker <- function(img_matrix, position, min_radius = 3, max_radius = 8, 
  ...
  hip_zone <- position[1] >= 25 && position[1] <= 50 && 
  ...
  min_brightness <- if(lower_leg_zone) {
    0.27  # More lenient for ankle/foot
  } else if(knee_zone) {
    0.50
  } else {
...
process_frame <- function(frame_path, target_size = 100, marker_size = 10, verbose = FALSE) {
  ...
  img_matrix <- img %>%
    image_convert("gray") %>%
    image_blur(radius = 1, sigma = 0.55) %>%
    image_data(channels = "gray") %>%
    as.numeric() %>%
...
output_dir <- "output_directory_S2_5_5_PD"
dir.create(output_dir, showWarnings = FALSE)

# Set up parameters
video_path <- "S2_5_5_PD.MOV"
...
#-------------------------------------------------------------------------------
# Check frame 595
frame_595 <- frames[595]  # Get the specific frame
test_result <- test_single_frame(frame_595, 
                                 target_size = 300, 
                                 marker_size = 10, 
                                 verbose = TRUE)
...
# Print the image matrix summary
cat("\nImage matrix summary:\n")
print(summary(as.vector(test_result$img_matrix)))
