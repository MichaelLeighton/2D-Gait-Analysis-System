...
find_brightness_threshold <- function(img_matrix, n_top = 10, verbose = FALSE) {
  ...
  threshold <- max(sorted_brightness[1] * 0.92, 0.8)
...
validate_marker <- function(img_matrix, position, min_radius = 3, max_radius = 8, 
  ...
  hip_zone <- position[1] >= 10 && position[1] <= 37 && 
    position[2] >= 155 && position[2] <= 210
  
  knee_zone <- position[1] >= 82 && position[1] <= 100 && 
    position[2] >= 122 && position[2] <= 205
  
  lower_leg_zone <- position[1] >= 130 && position[1] <= 158 && 
    position[2] >= 80 && position[2] <= 226
...
output_dir <- "output_directory_S4_2_5_normal"
dir.create(output_dir, showWarnings = FALSE)

# Set up parameters
video_path <- "S4_2_5_normal.MOV"
