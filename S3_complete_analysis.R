...
dir.create("gait_analysis/plots/S3", recursive = TRUE, showWarnings = FALSE)

# Load all processed data for the subject
files <- list.files("gait_analysis/processed_data", 
                    pattern = "S3_.*_processed.rds", 
                    full.names = TRUE)
...
analyze_temporal_parameters <- function(results_list) {
  ...
  threshold_lookup <- tribble(
    ...
    "limp",     2.5,    2.2,       0.3,          -0.04,     0.22,         0.08,
    "normal",   2.5,    2.4,       0.4,          -0.03,     0.22,         0.07,
    "PD",       2.5,    2.8,       0.3,          -0.02,     0.20,         0.10,
  ...
  ggsave("gait_analysis/plots/S3/S3_stance_raw_plots_y_adjusted_long.png", combined_plot, 
...
# Save the plot
ggsave("gait_analysis/plots/S3/S3_marker_trajectories_long.png", all_plots, 
       width = 10.66, height = 13.33, dpi = 300)
...
# Save the plot
ggsave("gait_analysis/plots/S3/S3_marker_trajectories_combined_long.png", all_combined_plots, 
       width = 10.66, height = 13.33, dpi = 300)
...
# Create Subject ID
kinematic_summary$subject_id <- "S3"  

# Save kinematic summary
write_csv(kinematic_summary, 
          "gait_analysis/complete_analysis/S3_kinematic_data.csv")
...
ggsave("gait_analysis/plots/S3/S3_kinematic_parameters.png", combined_kinematic_plots, 
...
temporal_summary$subject_id <- "S3" 

# Save temporal summary
write_csv(temporal_summary, 
          "gait_analysis/complete_analysis/S3_temporal_data.csv") 
...
# Save the combined plot
ggsave("gait_analysis/plots/S3/S3_temporal_parameters.png", combined_temp_plots, 
       width = 15, height = 15, dpi = 300)
