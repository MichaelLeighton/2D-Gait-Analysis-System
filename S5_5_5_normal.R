...
find_brightness_threshold <- function(img_matrix, n_top = 10, verbose = FALSE) {
  ...
  threshold <- max(sorted_brightness[1] * 0.972, 0.8)
...
validate_marker <- function(img_matrix, position, min_radius = 3, max_radius = 8, 
  ...
  hip_zone <- position[1] >= 25 && position[1] <= 45 && 
    position[2] >= 160 && position[2] <= 214
  
  knee_zone <- position[1] >= 82 && position[1] <= 105 && 
    position[2] >= 130 && position[2] <= 230
  
  lower_leg_zone <- position[1] >= 125 && position[1] <= 158 && 
  ...
  circularity_threshold <- if(lower_leg_zone) {
    0.526  # More lenient for ankle/foot
  } else if(knee_zone) {
...
process_frame <- function(frame_path, target_size = 100, marker_size = 10, verbose = FALSE) {
  ...
  img_matrix <- img %>%
    image_convert("gray") %>%
    image_blur(radius = 1, sigma = 0.32) %>%
    image_data(channels = "gray") %>%
    as.numeric() %>%
...
output_dir <- "output_directory_S5_5_5_normal"
dir.create(output_dir, showWarnings = FALSE)

# Set up parameters
video_path <- "S5_5_5_normal.MOV"
