...
find_brightness_threshold <- function(img_matrix, n_top = 10, verbose = FALSE) {
  ...
  threshold <- max(sorted_brightness[1] * 0.96, 0.8)
...
check_circularity <- function(img_matrix, position, radius = 5, threshold = 0.3, verbose = FALSE, is_lower_leg = FALSE) {
  ...
  min_points <- if(is_lower_leg) 17 else 12
...
validate_marker <- function(img_matrix, position, min_radius = 3, max_radius = 8, 
  ...
  hip_zone <- position[1] >= 10 && position[1] <= 45 && 
    position[2] >= 140 && position[2] <= 210
  
  knee_zone <- position[1] >= 77 && position[1] <= 100 && 
    position[2] >= 130 && position[2] <= 205
  
  lower_leg_zone <- position[1] >= 130 && position[1] <= 158 && 
    position[2] >= 80 && position[2] <= 240
  ...
  circularity_threshold <- if(lower_leg_zone) {
    0.51  # More lenient for ankle/foot
  } else if(knee_zone) {
...
process_frame <- function(frame_path, target_size = 100, marker_size = 10, verbose = FALSE) {
  ...
  frame_num <- as.numeric(gsub(".*?([0-9]+)\\.jpg$", "\\1", frame_path))
  
  # Define frames that require more/less blur 
  special_frames <- list(
    # "frames" = blur
    "19" = 0.8, 
    "21" = 0.8, 
  ...
  # Choose sigma based on frame number
  sigma_value <- if(!is.null(special_frames[[as.character(frame_num)]])) {
    special_frames[[as.character(frame_num)]]
  } else {
    0.42  # Default sigma value
  }
  ...
  img_matrix <- img %>%
    image_convert("gray") %>%
    image_blur(radius = 1, sigma = sigma_value) %>%
    image_data(channels = "gray") %>%
    as.numeric() %>%
  ...
  if(verbose) {
    ...
    print(paste("Using sigma value:", sigma_value))
  }
...
output_dir <- "output_directory_S4_5_5_limp"
dir.create(output_dir, showWarnings = FALSE)

# Set up parameters
video_path <- "S4_5_5_limp.MOV"
