...
validate_marker <- function(img_matrix, position, min_radius = 3, max_radius = 8, 
  ...
  lower_leg_zone <- position[1] >= 105 && position[1] <= 158 && 
  ...
  circularity_threshold <- if(lower_leg_zone) {
    0.21  # More lenient for ankle/foot
  } else if(knee_zone) {
...
output_dir <- "output_directory_S3_5_5_limp"
dir.create(output_dir, showWarnings = FALSE)

# Set up parameters
video_path <- "S3_5_5_limp.MOV"
