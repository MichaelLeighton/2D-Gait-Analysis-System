...
dir.create("gait_analysis/plots/S4", recursive = TRUE, showWarnings = FALSE)

# Load all processed data for the subject
files <- list.files("gait_analysis/processed_data", 
                    pattern = "S4_.*_processed.rds", 
                    full.names = TRUE)
...
analyze_temporal_parameters <- function(results_list) {
  ...
  threshold_lookup <- tribble(
    ...
    "limp",     2.5,    2.2,       0.15,         -0.015,    0.22,         0.08,
    "normal",   2.5,    2.0,       0.20,         -0.015,    0.22,         0.07,
    "PD",       2.5,    1.5,       0.9,         -0.015,     0.08,         0.05,
    "limp",     4.0,    1.8,       0.35,         -0.02,     0.20,         0.10,
    "normal",   4.0,    1.2,       0.9,          -0.07,     0.18,         0.12,
    "PD",       4.0,    1.8,       0.35,         -0.02,     0.18,         0.12,
  ...
  temporal_data <- lapply(results_list, function(result) {
    ...
    stance_data <- ankle_data %>%
      ...
      mutate(
        ...
        # Modified smoothing with condition-specific logic
        ankle_smooth = case_when(
          condition == "PD" & speed <= 3.0 ~ 
            rollapply(ankle_y, width = 5, mean, fill = NA, align = "center"),  # Wider smoothing for PD
          TRUE ~ 
            rollapply(ankle_y, width = 3, mean, fill = NA, align = "center")
        ),
        ...
        ankle_baseline = rollapply(ankle_y, width = 21, mean, fill = NA, align = "center"),
        
        # Detect local maxima
        local_max = ankle_y > lag(ankle_y, 1) & ankle_y > lead(ankle_y, 1),
        ...
        # Modified heel strike detection with condition-specific logic
        heel_strike = case_when(
          # Special case for limp at 2.5 km/h
          speed == 2.5 & condition == "limp" ~
            !is.na(ankle_y) &
            ankle_stable &
            ankle_y > ankle_baseline + (heel_height_threshold * 0.9) &  # Slightly more lenient
            !lag(ankle_stable, 1, default = FALSE) &
            lag(ankle_y_vel, 2, default = 0) < velocity_threshold,
          
          # Special case for PD at 2.5 km/h 
          speed == 2.5 & condition == "PD" ~ 
            !is.na(ankle_y) &
            ankle_stable &
            ((ankle_y > ankle_baseline + (heel_height_threshold * 1.8)) |  # More lenient threshold
               (local_max & 
                  ankle_y > ankle_baseline + (heel_height_threshold * 0.5) &   # Look for clear peaks
                  lag(ankle_y_vel, 2, default = 0) < velocity_threshold * 1.5)),
          
          # Keep all other existing conditions exactly as they were
          speed == 2.5 & condition == "normal" ~
            !is.na(ankle_y) &
            ankle_stable &
            ((ankle_y > ankle_baseline + heel_height_threshold) |
               (local_max & ankle_y > ankle_baseline + (heel_height_threshold * 0.8))) &
            !lag(ankle_stable, 1, default = FALSE) &
            lag(ankle_y_vel, 2, default = 0) < velocity_threshold,
          
          frame <= 100 & speed == 4.0 & condition %in% c("limp", "PD") ~
            !is.na(ankle_y) &
            ankle_stable &
            ((ankle_y > ankle_baseline + (heel_height_threshold * 0.8)) |
               (local_max & ankle_y > ankle_baseline + (heel_height_threshold * 0.6))) &
            !lag(ankle_stable, 1, default = FALSE) &
            lag(ankle_y_vel, 2, default = 0) < velocity_threshold,
      ...
      mutate(
        potential_toe_off = !is.na(foot_y_vel) &
          ...
          lead(foot_y_vel, 3, default = 0) > toe_curr_threshold &
          lead(foot_y_vel, 4, default = 0) > toe_curr_threshold &
          lead(foot_y_vel, 5, default = 0) > toe_curr_threshold,
    ...
    # Then handle the initial stance phase logic
  ...
  ggsave("gait_analysis/plots/S4/S4_stance_raw_plots_y_adjusted_long.png", combined_plot, 
...
#-------------------------------------------------------------------------------
#### COMBINED MARKER TRAJECTORY PLOTS
#-------------------------------------------------------------------------------
...
# Save the plot
ggsave("gait_analysis/plots/S4/S4_marker_trajectories_long.png", all_plots, 
       width = 10.66, height = 13.33, dpi = 300)
...
# Save the plot
ggsave("gait_analysis/plots/S4/S4_marker_trajectories_combined_long.png", all_combined_plots, 
       width = 10.66, height = 13.33, dpi = 300)
...
# Create Subject ID
kinematic_summary$subject_id <- "S4"  

# Save kinematic summary
write_csv(kinematic_summary, 
          "gait_analysis/complete_analysis/S4_kinematic_data.csv")
...
ggsave("gait_analysis/plots/S4/S4_kinematic_parameters.png", combined_kinematic_plots, 
...
temporal_summary$subject_id <- "S4" 

# Save temporal summary
write_csv(temporal_summary, 
          "gait_analysis/complete_analysis/S4_temporal_data.csv") 
...
# Save the combined plot
ggsave("gait_analysis/plots/S4/S4_temporal_parameters.png", combined_temp_plots, 
       width = 15, height = 15, dpi = 300)
